...
// patch took the patch over, and `mark` has no by= to give it back. The same call sliced the note to 80 characters
// without a word, dropping the half that said what was owed. Both are one mistake: a write that quietly changes what
// it was not asked to change. A place belongs to whoever made it, and a note that does not fit is refused out loud.
const long81 = 'x'.repeat(81)
for (const [title, args, expected] of [
  ['a place nobody marked before belongs to the marker', { by: 'Claude', note: 'my hut' }, { by: 'Claude', note: 'my hut' }],
  ['marking my own place again keeps my name', { saved: { by: 'Claude', note: 'old' }, by: 'Claude', note: 'new' }, { by: 'Claude', note: 'new' }],
  ['marking somebody else\'s place does NOT take it over', { saved: { by: 'Chani', note: 'hers' }, by: 'Claude', note: 'Chani\'s. 09-24 Claude: 2 cells owed' }, { by: 'Chani', note: 'Chani\'s. 09-24 Claude: 2 cells owed' }],
  ['no note given keeps the note already saved', { saved: { by: 'Chani', note: 'hers' }, by: 'Claude' }, { by: 'Chani', note: 'hers' }],
  ['no note and nothing saved is an empty note, not undefined', { by: 'Claude' }, { by: 'Claude', note: '' }],
  ['a note of exactly the limit fits', { by: 'Claude', note: 'x'.repeat(80) }, { by: 'Claude', note: 'x'.repeat(80) }],
  ['one character over is refused, and says how far over', { by: 'Claude', note: long81 },
    { error: 'note= is 81 characters and a place note holds 80: shorten it. Nothing was marked' }],
  ['a refused note saves nothing at all, not even the owner', { saved: { by: 'Chani', note: 'hers' }, by: 'Claude', note: long81 },
    { error: 'note= is 81 characters and a place note holds 80: shorten it. Nothing was marked' }]
]) {
  test(`markFields: ${title}`, () => assert.deepEqual(markFields(args), expected))
}

test('markFields: the limit the refusal names is the exported one', () => assert.equal(NOTE_MAX, 80))
...
for (const [name, args, expected] of [
  ...
  ['dark and roofed, two below the start: that is a cave mouth', { startY: 70, y: 68, skyLight: 0, light: 0 }, true],
  ['dark and roofed but level with the start: a shaft-lit cave floor, not a dive (13:20Z)', { startY: 56, y: 56, skyLight: 0, light: 0 }, false],
  ...
  ['a run that starts in a dark cave: the dark is where it already was', { startY: 60, y: 60, skyLight: 0, light: 0, startSkyLight: 0 }, false],
  ['from a dark cave, four further down still counts', { startY: 60, y: 56, skyLight: 0, light: 0, startSkyLight: 0 }, true]
]) {
...
// #125: `pen.check` walks from ONE cell and never said which. From 112,68,-126, outside claude-test-pen's fence, it
// answers `LEAKS via=116,68,-126`; from 114,68,-124, two blocks away and inside it, the same pen answers `holds
// cells=4`. Both readings are true of where they started and neither said where that was, which is how a LEAKS taken
// from outside got read as a hole in one's own fence, three times. So the cell is reported, and with it the answer to
// the question behind the confusion: was that cell inside a fence ring? Counted by crossings, the way you count
// whether a point lies inside any closed outline - odd going out means inside - on all four sides, because a gate is a
// hole in the ring and one ray through the gateway miscounts.
for (const [name, rows, expected] of [
  ['inside a shut ring, every side counts it in', ['      ', ' #### ', ' #S.# ', ' #..# ', ' #### ', '      '], 4],
  ['the ray through the gateway miscounts, the other three do not', ['      ', ' #### ', ' #S.# ', ' #..# ', ' #G## ', '      '], 3],
  ['two blocks outside a ring, the ray that meets it crosses twice', ['     ', ' ### ', 'S#.# ', ' ### ', '     '], 0],
  ['open country crosses nothing at all', ['S'], 0],
  ['a fence on two sides and nothing on the others cannot tell', ['#S', ' #'], 2]
]) {
  test(`penStance: ${name}`, () => assert.equal(penStance({ start: startOf(rows), topsAt: penMap(rows), radius: 4 }), expected))
}
for (const [name, from, votes, expected] of [
  ['inside on all four', '114,68,-124', 4, 'I walked from 114,68,-124, and the fences around it count that cell INSIDE a ring on 4 of the four sides'],
  ['inside on three, the fourth being the gateway', '114,68,-124', 3, 'I walked from 114,68,-124, and the fences around it count that cell INSIDE a ring on 3 of the four sides'],
  ['outside, and what that means for the reading', '112,68,-126', 0,
    'I walked from 112,68,-126, and the fences around it count that cell OUTSIDE any ring on 4 of the four sides: this is a reading of the ground outside the pen, so a leak found here says nothing about your fence. Check again from a cell inside it'],
  ['one side in, three out, still outside', '112,68,-126', 1,
    'I walked from 112,68,-126, and the fences around it count that cell OUTSIDE any ring on 3 of the four sides: this is a reading of the ground outside the pen, so a leak found here says nothing about your fence. Check again from a cell inside it'],
  ['two against two says it cannot tell, and does not guess', '112,68,-126', 2,
    'I walked from 112,68,-126, and the fences around it split two against two: I cannot tell whether that cell is inside the pen or outside it. Check again from a cell you know is inside']
]) {
  test(`stanceNote: ${name}`, () => assert.equal(stanceNote(from, votes), expected))
}
...
// #133: the craft loop counts from the LOCAL inventory and the server's answer lands after it has given up. A shears
// craft ate two iron ingots and still said "0/1 made"; a stone_axe craft said the same and had consumed nothing, and
// the very next identical call made the axe. Those are different accidents and only one of them costs anything, so
// what the message says about the ingredients is the part worth getting right.
const REJECTED = 'the server kept rejecting the craft'
for (const [name, args, expected] of [
  ['one asked for, one made', { item: 'stone_axe', count: 1, made: 1 }, { crafted: 'stone_axe', made: 1 }],
  ['a batch that overshoots is still a success', { item: 'stick', count: 1, made: 4 }, { crafted: 'stick', made: 4 }],
  ['nothing made and nothing consumed: cheap, retry', { item: 'stone_axe', count: 1, made: 0, why: REJECTED }, {
    error: 'the server kept rejecting the craft: no stone_axe made at all and nothing was consumed, so nothing is lost: retry once, the second call usually works'
  }],
  ['nothing made and the ingredients gone: that loss is real', { item: 'shears', count: 1, made: 0, spent: { iron_ingot: 2 }, why: REJECTED }, {
    error: 'the server kept rejecting the craft: no shears made at all and the ingredients are gone (iron_ingot:2): the server took them and gave nothing back, so this loss is real. Check your inventory before retrying'
  }],
  ['part-way: what it used became the result, so nothing is called lost', { item: 'cobblestone_stairs', count: 100, made: 4, spent: { cobblestone: 6 }, why: 'the ingredients ran out' }, {
    error: 'the ingredients ran out: only 4 of 100 cobblestone_stairs made, and cobblestone:6 went into them'
  }],
  ['part-way with nothing read back about the ingredients', { item: 'stick', count: 8, made: 4, why: 'the ingredients ran out' }, {
    error: 'the ingredients ran out: only 4 of 8 stick made'
  }],
  ['an ingredient that did not move is not named', { item: 'shears', count: 1, made: 0, spent: { iron_ingot: 2, stick: 0 }, why: REJECTED }, {
    error: 'the server kept rejecting the craft: no shears made at all and the ingredients are gone (iron_ingot:2): the server took them and gave nothing back, so this loss is real. Check your inventory before retrying'
  }],
  ['every ingredient still in my pockets reads as consumed nothing', { item: 'shears', count: 1, made: 0, spent: { iron_ingot: 0 }, why: REJECTED }, {
    error: 'the server kept rejecting the craft: no shears made at all and nothing was consumed, so nothing is lost: retry once, the second call usually works'
  }]
]) {
  test(`craftReport: ${name}`, () => assert.deepEqual(craftReport(args), expected))
}
...
// mineflayer-pathfinder 2.4.5 lib/goto.js: goto resolves on the first path_update with an empty path, partial ones included. From a dead end
// that is the nearest cell to the goal (the alley beside Perrin's sheep pen gate) the first 40 ms slice has no step yet: goto answered
// "no walkable path" in a second, and the pathfinder, goal still set, went on searching and walked the body off after the task had failed
...
for (const [name, source, expected] of [
  ['the upstream source waits out a partial search', `a\n${GOTO_EMPTY}b`, { status: 'patched', source: `a\n${GOTO_FIXED}b` }],
  ['patched already: left alone', `a\n${GOTO_FIXED}b`, { status: 'already', source: `a\n${GOTO_FIXED}b` }],
  ['a new upstream version without that code: say so, change nothing', 'something else', { status: 'anchor missing', source: 'something else' }]
]) {
  test(`patchGotoPartial: ${name}`, () => assert.deepEqual(patchGotoPartial(source), expected))
}
...
for (const [name, args, expected] of [
  ...
  ['rotten flesh is carried and never eaten', { food: 12, carried: sorted([], ['rotten_flesh']) },
    'nothing I carry is food (never eaten: rotten_flesh, which I eat only at food 6 or less or with anyway=true)'],
  ...
  ['both reasons, both said', { food: 12, carried: sorted([], ['rotten_flesh'], ['cobblestone']) },
    'nothing I carry is food (never eaten: rotten_flesh, which I eat only at food 6 or less or with anyway=true; not food: cobblestone)'],
  ...
  ['the item I named is on the never-eat list', { food: 12, item: 'rotten_flesh', carried: sorted(['bread'], ['rotten_flesh']) },
    'no rotten_flesh I would eat: it is on the never-eat list, which I eat only at food 6 or less or with anyway=true; what I carry is bread'],
  ...
  // backlog #139: a body at food 3 carrying 8 rotten flesh was told it carried nothing edible, and starved holding
  // its own dinner. Hunger damage stops at half health on Normal, so the flesh costs nothing the starving did not.
  ['at the floor with nothing but flesh, the flesh is dinner', { food: 6, carried: sorted([], ['rotten_flesh']) }, null],
  ['one above the floor it is still refused', { food: 7, carried: sorted([], ['rotten_flesh']) },
    'nothing I carry is food (never eaten: rotten_flesh, which I eat only at food 6 or less or with anyway=true)'],
  ['at the floor with real food, the real food wins and the flesh stays refused', { food: 4, item: 'rotten_flesh', carried: sorted(['bread'], ['rotten_flesh']) },
    'no rotten_flesh I would eat: it is on the never-eat list, which I eat only at food 6 or less or with anyway=true; what I carry is bread'],
  ['anyway=true eats the flesh whatever the belly says', { food: 12, item: 'rotten_flesh', anyway: true, carried: sorted(['bread'], ['rotten_flesh']) }, null],
  ['anyway=true with no item named and nothing else', { food: 12, anyway: true, carried: sorted([], ['rotten_flesh']) }, null],
  ['anyway=true cannot conjure food out of empty pockets', { food: 12, anyway: true, carried: sorted() },
    'nothing I carry is food: my pockets are empty'],
  ['anyway=true does not beat a full belly', { food: 20, anyway: true, carried: sorted([], ['rotten_flesh']) },
    'food is already 20: the game refuses a meal at a full belly'],
  ['at the floor the flesh is named as what I carry', { food: 2, item: 'cake', carried: sorted([], ['rotten_flesh']) },
    'no cake I would eat: I do not carry it; what I carry is rotten_flesh'],
  ['a different floor can be asked for', { food: 9, floor: 10, carried: sorted([], ['rotten_flesh']) }, null]
]) {
  test(`eatRefusal: ${name}`, () => assert.equal(eatRefusal(args), expected))
}

// The reflex has to know WHAT to put in the hand, not only that it may: mineflayer-auto-eat keeps its own banned list,
// so a desperate meal is only eaten when the item is handed to it by name.
for (const [name, args, expected] of [
  ['well fed, only the ordinary food is on the table', { food: 15, carried: sorted(['bread'], ['rotten_flesh']) }, { desperate: false, allowed: ['bread'] }],
  ['at the floor with nothing else, the banned food joins it', { food: 6, carried: sorted([], ['rotten_flesh']) }, { desperate: true, allowed: ['rotten_flesh'] }],
  ['at the floor with bread in hand, nothing changes', { food: 6, carried: sorted(['bread'], ['rotten_flesh']) }, { desperate: false, allowed: ['bread'] }],
  ['anyway=true puts both on the table', { food: 15, anyway: true, carried: sorted(['bread'], ['rotten_flesh']) }, { desperate: true, allowed: ['bread', 'rotten_flesh'] }],
  ['empty pockets stay empty', { food: 1, carried: sorted() }, { desperate: true, allowed: [] }]
]) {
  test(`eatAllowed: ${name}`, () => assert.deepEqual(eatAllowed(args), expected))
}

test('eatAllowed: the floor is the one the refusal names', () => assert.equal(HUNGER_FLOOR, 6))
...
// api.until: counted in the ticks actually waited, never on the wall clock. darkstar suspends when idle (09-24): a
// Date.now() deadline fired every routine's "the day never ended" after a 46-minute freeze in which nothing happened.
const untilRun = async (answers, opts) => {
  const waits = []
  const until = makeUntil({ waitTicks: async n => { waits.push(n) }, alive: () => {}, composite: 'routine' })
  const pred = () => answers.shift() ?? false
  const outcome = await until(pred, opts).then(r => r, e => e.message)
  return { outcome, waits }
}

for (const [name, answers, opts, expected] of [
  ['true at once waits for no ticks', [true], { timeout: 3 }, { outcome: true, waits: [] }],
  ['true on the third look waited two rounds of every', [false, false, true], { timeout: 60, every: 10 }, { outcome: true, waits: [200, 200] }],
  ['timeout is counted in rounds waited, not clock time', [], { timeout: 3, every: 1, what: 'the day never ended' }, { outcome: 'routine: waited 3s and the day never ended', waits: [20, 20, 20] }],
...
// ---------------------------------------------------------------- #140: which code is this body actually running
// Two implementers share this tree, so a body started between two saves runs the new helper with the old caller and
// throws something that is in nobody's diff. Mine did: `eat` threw `Cannot destructure property 'edible'` against a
// tree that was mid-commit, not broken. The join line says the commit, and says out loud when the body is not running
// it as committed - but only for files a body LOADS, so an edited guide never cries wolf.
for (const [name, input, expected] of [
  ['a clean checkout says just the commit', { head: 'f7777df', changed: [] },
    { code: 'f7777df' }],
  ['only docs edited is still a clean body', { head: 'f7777df', changed: ['AGENT_GUIDE.md', 'state/BUGS.md'] },
    { code: 'f7777df' }],
  ['one edited source file', { head: 'f7777df', changed: ['src/lib.mjs'] },
    { code: 'f7777df+1', dirty: 'src/lib.mjs' }],
  ['a composite counts too, and they are sorted', { head: 'f7777df', changed: ['library/farm/harvest.mjs', 'src/bot.mjs'] },
    { code: 'f7777df+2', dirty: 'library/farm/harvest.mjs src/bot.mjs' }],
  ['a long list stops at four', { head: 'f7777df', changed: ['src/a.mjs', 'src/b.mjs', 'src/c.mjs', 'src/d.mjs', 'src/e.mjs', 'src/f.mjs'] },
    { code: 'f7777df+6', dirty: 'src/a.mjs src/b.mjs src/c.mjs src/d.mjs and 2 more' }],
  ['no git at all', { head: null, changed: [] }, { code: 'unknown' }]
]) {
  test(`codeVersion: ${name}`, () => {
    const { advice, ...said } = codeVersion(input)
    assert.deepEqual(said, expected)
  })
}

test('codeVersion: a dirty body says the commit is not what it is running, and where to look', () => {
...
test('codeVersion: a clean body has nothing to advise', () => {
  assert.equal(codeVersion({ head: 'abc1234', changed: [] }).advice, undefined)
})
...
// ---------------------------------------------------------------- #119: the kicked line that never stopped
// A body refused at login is kicked again on every retry, every ten seconds, for ever. `disconnected` is gated and
// `kicked` was not, so a throwaway body of mine that was not on the whitelist would have written one line all night.
// The repeat state was also a single slot shared by every kind of message, so two faults taking turns each reset the
// other's window and neither was ever suppressed: one slot per kind fixes that too.
test('repeatByType: the first of a kind is always said', () => {
  assert.equal(repeatByType(null, 'kicked', 'not whitelisted', 1000).say, 'not whitelisted')
})

test('repeatByType: the same message again inside the window is counted, not said', () => {
  const first = repeatByType(null, 'kicked', 'not whitelisted', 1000)
  assert.equal(repeatByType(first.seen, 'kicked', 'not whitelisted', 11000).say, null)
})

test('repeatByType: a kick that says something NEW is said at once', () => {
  const first = repeatByType(null, 'kicked', 'not whitelisted', 1000)
  assert.equal(repeatByType(first.seen, 'kicked', 'banned', 11000).say, 'banned')
})

test('repeatByType: two kinds taking turns do not reset each other', () => {
  let state = null
  state = repeatByType(state, 'kicked', 'not whitelisted', 1000).seen
  state = repeatByType(state, 'error', 'something else', 2000).seen
...
test('repeatByType: a kick loop says its count far more rarely than an ordinary error', () => {
  ...
  // a login refusal retries every ten seconds: a minute of them is still silence
  assert.equal(repeatByType(first.seen, 'kicked', 'not whitelisted', 90000).say, null)
  const said = repeatByType(first.seen, 'kicked', 'not whitelisted', REPEAT_WINDOW.kicked + 1).say
  assert.match(said, /not whitelisted \(1 more in the last \d+s\)/)
})
...
// Chani stopped mine.get and farm.maintain as wedged: `asleep doing=mine.get 174s` with nothing moving. They were not:
// a composite sleeps through the night at its next checkpoint and goes on at dawn, and a night where somebody stays
// awake lasts ~9 minutes. The doing= line now says so, and how to take the body back
for (const [name, args, expected] of [
  ['no task', {}, null],
  ['a task at work', { name: 'mine.get', seconds: 12 }, 'mine.get 12s'],
  ['a composite asleep for the night', { name: 'mine.get', seconds: 174, paused: 'night' },
    'mine.get 174s, paused: asleep for the night, goes on at dawn by itself (./mc stop takes the body back now)']
]) {
...
// my own hole-up in the cave under the test pits (13:17Z): dug three down, capped, and the shaft stood open to the cave
// on two sides. A hole is closed on all four sides at feet and head height and over the head, however it was made
test('holeCells: every hole is walled at feet and head height and capped', () => assert.deepEqual(
  holeCells().map(c => c.join(',')).sort(),
  ['-1,0,0', '-1,1,0', '0,0,-1', '0,0,1', '0,1,-1', '0,1,1', '0,2,0', '1,0,0', '1,1,0'].sort()))

// my own craft item=stick at 121,90,-198 (13:30Z): one stick made, "-bamboo:26", and the next craft got 24 bamboo back.
// A 2x2 craft whose put-back click the server rejected leaves the stack on the cursor or in the grid, where the pockets
// count never looks. Name what is stuck there so the craft can hand it back and say so
for (const [name, args, expected] of [
  ['nothing stuck', { grid: [null, null, null, null], cursor: null }, {}],
  ['the stack left on the cursor', { grid: [null, null, null, null], cursor: { name: 'bamboo', count: 24 } }, { bamboo: 24 }],
  ['coal left in two grid cells', { grid: [{ name: 'coal', count: 4 }, null, { name: 'coal', count: 3 }, null], cursor: null }, { coal: 7 }],
  ['grid and cursor together', { grid: [{ name: 'coal', count: 1 }, null, null, null], cursor: { name: 'stick', count: 2 } }, { coal: 1, stick: 2 }]
]) {
  test(`gridLeftovers: ${name}`, () => assert.deepEqual(gridLeftovers(args), expected))
}

// my own body, 13:40Z at 106,64,-130: the flee from two zombies and a skeleton stopped short of the cave and holed up.
// Digging three down took eight seconds with all three hitting it, and it died at the bottom with a stone sword in its
// pack. A hostile in reach turns a hole-up into standing still to be killed: an armed body fights instead
for (const [name, args, expected] of [
  ['armed, a zombie two blocks off: fight, do not dig', { armed: true, hostileDist: 2 }, 'fight'],
  ['armed, a zombie right at the edge of reach: fight', { armed: true, hostileDist: 4 }, 'fight'],
  ['armed, the nearest mob is 6 off: time to dig', { armed: true, hostileDist: 6 }, null],
  ['armed, nothing about: dig', { armed: true, hostileDist: Infinity }, null],
  ['unarmed with a zombie on top of me: digging is all that is left', { armed: false, hostileDist: 2 }, null]
]) {
  test(`holeUpRefusal: ${name}`, () => assert.equal(holeUpRefusal(args), expected))
}

// my own goto (13:27Z) ended "lost bread:1": the body ate it on the way. A meal is not a loss; the tally says ate= instead
for (const [name, args, expected] of [
  ['no meal: the tally is untouched', { gained: { dirt: 2 }, lost: { cobblestone: 5 }, ate: {} }, { gained: { dirt: 2 }, lost: { cobblestone: 5 } }],
  ['the bread I ate is not lost', { gained: {}, lost: { bread: 1, cobblestone: 5 }, ate: { bread: 1 } }, { gained: {}, lost: { cobblestone: 5 }, ate: { bread: 1 } }],
  ['ate two, lost three: one of them really went', { gained: {}, lost: { bread: 3 }, ate: { bread: 2 } }, { gained: {}, lost: { bread: 1 }, ate: { bread: 2 } }],
  ['ate what the task also picked up: still a meal, nothing lost', { gained: {}, lost: {}, ate: { carrot: 1 } }, { gained: {}, lost: {}, ate: { carrot: 1 } }]
]) {
  test(`mealTally: ${name}`, () => assert.deepEqual(mealTally(args), expected))
}
test('terse: a task that ate says so on its own line item', () =>
  assert.equal(terse({ ok: true, task: 3, action: 'goto', seconds: 79, gained: {}, lost: { cobblestone: 5 }, ate: { bread: 1, carrot: 2 }, pos: { x: 1, y: 2, z: 3 } }),
    'ok goto 79s -cobblestone:5 ate=bread:1,carrot:2 @1,2,3'))

// a mine.get *_log cut a corner post of a cherry village house at 61,68,-184 and put it back on the wrong axis. A log is
// a tree's only when leaves grow within 2 of it or of the top of its log column, and nothing built touches that column
const world = cells => ({ x, y, z }) => cells[`${x},${y},${z}`] ?? 'air'
const trunk = (height, top) => Object.fromEntries([...Array.from({ length: height }, (_, i) => [`0,${i},0`, 'oak_log']), ...top.map(([c, n]) => [c, n])])
for (const [name, cells, at, expected] of [
  ['the foot of a tall oak: leaves over the top of its column', trunk(6, [['1,6,0', 'oak_leaves'], ['0,6,0', 'oak_leaves']]), { x: 0, y: 0, z: 0 }, true],
  ['a log with leaves right beside it', trunk(1, [['2,0,0', 'cherry_leaves']]), { x: 0, y: 0, z: 0 }, true],
  ['a house post: stairs on top, no leaves', trunk(3, [['0,3,0', 'cherry_stairs']]), { x: 0, y: 0, z: 0 }, false],
  ['a post by a tree: leaves near, but planks touch the column', trunk(2, [['1,1,1', 'cherry_planks'], ['2,2,0', 'cherry_leaves']]), { x: 0, y: 0, z: 0 }, false],
  ['a lone log on the ground (a stump or a dropped beam): no leaves, not a tree', trunk(1, []), { x: 0, y: 0, z: 0 }, false]
]) {
  test(`isTreeLog: ${name}`, () => assert.equal(isTreeLog(at, world(cells)), expected))
}
for (const [name, over, expected] of [
  ['placed logs are passed over and counted', { placed: p => p === 'a' || p === 'b' }, { found: ['c', 'd'], skippedPlaced: 2 }],
  ['every one of them placed', { placed: () => true }, { error: "the only sand within 48 blocks is placed wood (someone's build: posts, beams), not trees: go where trees grow and retry" }]
]) {
  test(`mineTargets placed: ${name}`, () => assert.deepEqual(pick(over), expected))
}

// path_to said gates=3 for Perrin's cow pen and nothing about WHICH three: the walk went round and round a birch pen
// 15 blocks off (13:58Z). The route names its gates and a waypoint every six steps, so a detour shows before the walk
const node = (x, y, z, gate) => ({ x, y, z, ...(gate ? { toPlace: [{ x: gate[0], y: gate[1], z: gate[2], useOne: true }] } : {}) })
for (const [name, path, expected] of [
  ['no path at all', [], { gatesAt: '', through: '' }],
  ['a short walk: the last step is always named', [node(1, 64, 1), node(2, 64, 1)], { gatesAt: '', through: '2,64,1' }],
  ['a gate on the way and a long walk', Array.from({ length: 13 }, (_, i) => node(i, 64, 0, i === 3 ? [3, 64, 1] : null)),
    { gatesAt: '3,64,1', through: '5,64,0 11,64,0 12,64,0' }],
  ['a toPlace that is a real block, not a gate, is no gate', [node(0, 64, 0, null), { x: 1, y: 65, z: 0, toPlace: [{ x: 1, y: 64, z: 0, useOne: false }] }], { gatesAt: '', through: '1,65,0' }]
]) {
  test(`routeSummary: ${name}`, () => assert.deepEqual(routeSummary(path), expected))
}

// Perrin's goto to his cow pen gate ran 170-290 s and ended 20 blocks further off; mine circled a birch pen every 11 s
// (13:58Z). It never stood still, so the 12 s stall alarm never rang. A walk that gets no nearer than its best for 40 s
// is going round in circles: end it and say why
for (const [name, args, expected] of [
  ['closing in', { dist: 12, best: 13, bestAgeMs: 1000 }, false],
  ['a detour that is still young', { dist: 20, best: 3, bestAgeMs: 30000 }, false],
  ['40 s and never nearer than 2 off: circling', { dist: 15, best: 2, bestAgeMs: 40000 }, true],
